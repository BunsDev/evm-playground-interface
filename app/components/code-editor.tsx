...
const CodeEditor: FC<CodeEditorProps> = ({
    ...
    // handles: window resize for responsive editor
    useEffect(() => {
        const handleResize = () => {
    ...
    const handleEditorDidMount = async (
        ...
        // configures monaco for ts: enable top-level await + ESM + Node-like resolution
        monacoInstance.languages.typescript.typescriptDefaults.setCompilerOptions({
            ...
            allowJs: true,
            // makes libs modern so TS allows top-level await in modules
            lib: ["es2022", "dom"],
            // helps TS: resolve bare specifiers via virtual node_modules files we add below
            baseUrl: "file:///",
            paths: {
            ...
            skipLibCheck: true,
            strict: false,
            // enables: better import resolution
            allowImportingTsExtensions: true,
            noImplicitAny: false,
        });

        // configures: editor options for better auto-import experience
        monacoInstance.languages.typescript.typescriptDefaults.setEagerModelSync(
            true
        );
        ...
        // extracts: viem functions dynamically from type definitions with correct import paths
        const extractViemFunctions = () => {
            const functionMap = new Map<string, string>(); // function name -> import path

            // gets: all the viem type files we've loaded
            const viemTypeFiles = [
            ...
            // extracts: function names from type definitions with their import paths
            viemTypeFiles.forEach((filePath) => {
                ...
                if (content) {
                    // determines: the import path based on file location
                    let importPath = "viem"; // default to main viem

                    if (filePath.includes("/_types/")) {
                        // extracts: submodule from path like /_types/accounts/index.d.ts
                    ...
                    // matches: function declarations and exports
                    ...
                    // matches: exported const functions
        ...
        if (typeof globImport !== "function") {
            console.warn(
                "Monaco setup: import.meta.glob is not available; viem types will not be preloaded."
            );
        }

        const viemDtsFiles: Record<string, string> =
            typeof globImport === "function"
                ? (globImport("/node_modules/viem/_types/**/*.d.ts", {
                      query: "?raw",
                      import: "default",
                      eager: true,
        ...
        // also includes: the TypeScript sources so module paths like 'viem/chains' resolve fully
        const viemTsFiles: Record<string, string> =
            typeof globImport === "function"
                ? (globImport("/node_modules/viem/_types/**/*.ts", {
                      query: "?raw",
                      import: "default",
                      eager: true,
                  }) as Record<string, string>)
        ...
        // creates: lightweight entry points that re-export the real type trees.
        // aligns with imports: like `import {...} from 'viem'` and 'viem/chains'
        monacoInstance.languages.typescript.typescriptDefaults.addExtraLib(
        ...
        const viemSubmoduleIndexFiles: Record<string, string> =
            typeof globImport === "function"
                ? (globImport("/node_modules/viem/_types/**/index.ts", {
                      query: "?raw",
                      import: "default",
                      eager: true,
                  }) as Record<string, string>)
        ...
        // also generates: wrappers for subpaths that only have index.d.ts (e.g., chains)
        const viemSubmoduleIndexDtsFiles: Record<string, string> =
            typeof globImport === "function"
                ? (globImport("/node_modules/viem/_types/**/index.d.ts", {
                      query: "?raw",
                      import: "default",
                      eager: true,
                  }) as Record<string, string>)
        ...
        // configures: additional editor features for better auto-import experience
        // note: setIncludePackageJsonAutoImports is not available in this Monaco version

        // adds: some common Node.js types for better auto-imports
        monacoInstance.languages.typescript.typescriptDefaults.addExtraLib(
    ...
    // locates: a reasonable line for a variable key in the current source
    ...
    // updates: glyph margin markers when code/logs change
    useEffect(() => {
