...
export const ScriptManager: FC<ScriptManagerProps> = ({
    onScriptLoad,
    onScriptCreate,
    currentScript,
}) => {
                ...
                <div className="p-4 space-y-2">
                    {scripts.length === 0 ? (
                        ...
                        scripts.map((script) => (
                            <motion.div
                                key={script.id}
                                className={`p-3 border rounded-lg transition-colors hover:bg-muted/50 cursor-pointer ${currentScript?.id === script.id
